...
reportDomain='http://YOURDOMAIN.COM/datalog.php'

##################################################################
### If you would like to disable logging just change this to 0 ###
### To enable logging set this to 1                            ###
##################################################################
enablelog=0
...
##
# Functions that search for outdated WordPress installations in /home/*
...
function find_outdated_wp() {
    ...
    echo ""
        ionice -n 3 -c 3 2>/dev/null find /home ! -path "/home/virtfs/*" -type f -iwholename "*/wp-includes/version.php" -exec grep -H "\$wp_version =" {} >> $tempf1 \; &
    ...
    sleep 2
        ...
        cat $tempf2 | column -s '|' -s '|' -t > $output
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
    1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=AccessLogsForDomain\&Server=$server\&Path=$location ; fi ; MenuAcessDomain;;
    2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=AccessLogsFromSpecificIPForDomain\&Server=$server\&Path=$location ; fi ; MenuAcessSpecificIPForDomain;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=AccessAndIPLogs\&Server=$server\&Path=$location ; fi ; access_and_ip_logs;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=OnlyAccessLogs\&Server=$server\&Path=$location ; fi ; OnlyAccessLogs;;
    5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ApacheErrorsWebSite\&Server=$server\&Path=$location ; fi ; domainhttpderrors;;
    6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ApacheErrorsUsername\&Server=$server\&Path=$location ; fi ; userhttpderrors;;
                7) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CheckSpike\&Server=$server\&Path=$location ; fi ; check_spike_date;;
    8) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CheckCurrentSpike\&Server=$server\&Path=$location ; fi ; check_spike_current;;
    0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
while [ -z $ckituser ] ; do
  ...
  echo ""
  echo "To start please enter your ckituser (this is part of the logging setup - to start you can enter ckituser):"
    read ckituser
    if ([[ ! $ckituser =~ [a-z_]+$ ]] || [[ ! $ckituser =~ ^ckit[a-z_]+$ ]]) || [[ ${#ckituser} -lt 6 ]] ; then
                unset ckituser
                echo "Don't cheat! Enter your correct ckituser!"
    fi
done
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CloudQuickInstallMenu\&Server=$server\&Path=$location ; fi ; CloudQuickInstallMenu;;
    2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=PHPchangesMenu\&Server=$server\&Path=$location ; fi ; ChangePHPVersion;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=IsExtensionEnabled\&Server=$server\&Path=$location ; fi ; is_extensionCloud;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=IsFunctionnEnabled\&Server=$server\&Path=$location ; fi ; is_functionCloud;;
    5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=RandomPass\&Server=$server\&Path=$location ; fi ; randompass_cloud;;
    6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=IonCubeInstaller\&Server=$server\&Path=$location ; fi ; install_ioncube_php70;;
    7) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=FixWordPressWebsites\&Server=$server\&Path=$location ; fi ; FixWordpressWebsite;;
    0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exit\&Server=$server\&Path=$location ; fi ; Exitmenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=InstallwpCLI\&Server=$server\&Path=$location ; fi ; wp_cli_cloud_install;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=InstallComposer\&Server=$server\&Path=$location ; fi ; composer_cloud_install;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=InstallLaravel\&Server=$server\&Path=$location ; fi ; laravel_cloud_installer;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=InstallMagento2OnTheCloud\&Server=$server\&Path=$location ; fi ; mage2_install_cloud;;
    5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=InstallWordpressOnCloud\&Server=$server\&Path=$location ; fi ; wp_install_cloud;;
    0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CloudMenu\&Server=$server\&Path=$location ; fi ; CloudMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo5.3\&Server=$server\&Path=$location ; fi ; changePHPTo5.3;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo5.4\&Server=$server\&Path=$location ; fi ; changePHPTo5.4;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo5.5\&Server=$server\&Path=$location ; fi ; changePHPTo5.5;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo5.6\&Server=$server\&Path=$location ; fi ; changePHPTo5.6;;
                5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo7\&Server=$server\&Path=$location ; fi ; changePHPTo7;;
              6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangePHPVersionTo7.1\&Server=$server\&Path=$location ; fi ; changePHPTo71;;
    7) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Deploy_optimized_PHP_ini\&Server=$server\&Path=$location ; fi ; DeployPHPini;;
                8) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ChangeShellPHPVersion\&Server=$server\&Path=$location ; fi ; ChangeShellPHP;;
                0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; CloudMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim1ListofAllEmailSenders\&Server=$server\&Path=$location ; fi ; showexim ;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim2EximSpamDirs\&Server=$server\&Path=$location ; fi ; originate;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim3EximPHPSpam\&Server=$server\&Path=$location ; fi ; originate2;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim4EximUsersSpam\&Server=$server\&Path=$location ; fi ; whichphpscript;;
                5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim5IPsOnPort25\&Server=$server\&Path=$location ; fi ; getnetstat;;
                6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim6NobodySpam\&Server=$server\&Path=$location ; fi ; nobodyspam;;
                7) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim7SpamInProgress\&Server=$server\&Path=$location ; fi ; nobodyspamafter;;
                8) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exim8MailQueue\&Server=$server\&Path=$location ; fi ; showeximsum;;
                0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
SysAdminsMenu(){
  ...
  if [ $syspass = "exit" ]; then
                unset syspass
                MainMenu
  elif [ $syspass != "SysPass" ]; then
    echo "Wrong Password!"
    unset syspass
  fi
    done
    if [ $syspass = "SysPass" ]; then
  if [[ $(pwd | grep '/var/sites/') ]]; then
  echo $(ColorRed 'You are not on cPanel')
  WrongCommand
  MainMenu
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=List_sleeping_mysql_processes\&Server=$server\&Path=$location ; fi ; list_sleeping_mysql_admins;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Kill_mysql_sleeping_processes\&Server=$server\&Path=$location ; fi ; kill_mysql_sleeping_proc;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Show_ll_rocesses\&Server=$server\&Path=$location ; fi ; show_full_processlist_admins;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Kill_mysql_sleeping_processes_for_specific_user\&Server=$server\&Path=$location ; fi ; kill_mysql_sleeping_proc_user;;
                0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=List_sleeping_mysql_processes\&Server=$server\&Path=$location ; fi ; list_sleeping_mysql;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Show_ll_rocesses\&Server=$server\&Path=$location ; fi ; show_full_processlist;;
          1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MySQL_status_and_connections\&Server=$server\&Path=$location ; fi ; mysql_status;;
                0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=IsExtensionEnabled\&Server=$server\&Path=$location ; fi ; is_extension;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=IsFunctionEnabled\&Server=$server\&Path=$location ; fi ; is_function;;
    3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=RandomPass\&Server=$server\&Path=$location ; fi ; randompass;;
    4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MonitorCPU\&Server=$server\&Path=$location ; fi ; MonitorCpu;;
    5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=FindLargeFiles\&Server=$server\&Path=$location ; fi ; FindLargeFiles;;
    6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=EAversion\&Server=$server\&Path=$location ; fi ; EAversion;;
                7) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CheckOutdatedWP\&Server=$server\&Path=$location ; fi ; find_outdated_wp;;
    0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
    1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ActiveConn\&Server=$server\&Path=$location ; fi ; ActiveConn;;
                2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=TopUsers\&Server=$server\&Path=$location ; fi ; TopUsers;;
                3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=AllUsers\&Server=$server\&Path=$location ; fi ; AllUsers;;
                4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CPUusage\&Server=$server\&Path=$location ; fi ; CurrentCPUusage;;
                5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ConnectionsOnSpecPort\&Server=$server\&Path=$location ; fi ; GetPortConn;;
    6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=ResourceUsagePerUser\&Server=$server\&Path=$location ; fi ; Zack;;
                0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MainMenu\&Server=$server\&Path=$location ; fi ; MainMenu;;
...
$(ColorBlue 'Choose an option:') "
                read a
                case $a in
                1) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MenuAccess\&Server=$server\&Path=$location ; fi ; MenuAcess;;
    2) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=EmailsMenu\&Server=$server\&Path=$location ; fi ; EmailsMenu;;
    3) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=MySQLMenu\&Server=$server\&Path=$location ; fi ; MySQLMenu;;
    4) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=WebTrafficMenu\&Server=$server\&Path=$location ; fi ; DDoSMenu;;
    5) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=HandyToolsMenu\&Server=$server\&Path=$location ; fi ; ToolsMenu;;
#   6) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=CloudMenu\&Server=$server\&Path=$location ; fi ; CloudMenu;;
    admins) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=SysAdminsMenu\&Server=$server\&Path=$location ; fi ; SysAdminsMenu;;
    0) if [[ $enablelog == 1 ]] ; then curl ${reportDomain}?user=$ckituser\&Date=$executionTime\&Executed=Exit\&Server=$server\&Path=$location ; fi ; Exitmenu;;
    *) echo -e $red"Wrong command."$clear; WrongCommand;;
        esac
}
